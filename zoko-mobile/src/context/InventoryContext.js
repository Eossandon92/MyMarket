    ...
    const fetchProducts = useCallback(async () => {
        ...
        try {
            const [resProd, resPromo] = await Promise.all([
                ...
                fetch(`${API}/promotions?business_id=${businessId}`, { headers: { "Authorization": `Bearer ${token}` } }).catch(() => null)
            ]);
            ...
            if (resPromo && resPromo.ok) {
                const prData = await resPromo.json();
                allProducts = [...allProducts, ...prData];
            }
